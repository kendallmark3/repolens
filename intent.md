...
> Mirrors [`repo-scope/intent/project-intent.md`](repo-scope/intent/project-intent.md), the working source of truth used by the Intent-Driven Engineering loop inside `repo-scope/`. Kept here so the intent is visible from the workspace root without opening the subproject.
...
## Success Criteria
Full detailed specification (data entities, screens, seed requirements, validation checklist) is preserved verbatim in [`repo-scope/intent/current-feature.md`](repo-scope/intent/current-feature.md) as the working source of truth for this build.
